...
import {
  newReplaceWith,
  newToBeReplaced,
} from "~/features/texts/find-in-text-slice";

import { WordMatchHelper } from "../settings/helper-word-match";

interface FindInTextProps {
  tool: ToolKeys;
}

export const FindInTextReplaceHelper: FC<FindInTextProps> = ({ tool }) => {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <WordMatchHelper debouncedFunction={changeOutputValue} />
    </div>
  );
};

export const FindInTextFindHelper: FC<FindInTextProps> = ({ tool }) => {
  const findInTextHelper = useSelector(
    (state: AppState) => state.findInTextHelper,
  );
  const inputValue = useSelector((state: AppState) => state.input);
